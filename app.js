...
function allItems(){return catalog?[...catalog.news,...catalog.papers,...catalog.topics]:[];}
...
function card(item,index=0,featured=false){const isTopic=item.type==='topic',image=imageUrl(item),date=item.published||item.date;
return `<article class="card ${featured?'featured':''} ${isTopic?'topic-card':''}"><div class="card-top"><span class="badge ${badgeClass(item)}">${esc(item.category)}</span><span>${isTopic?'研究专题':item.type==='paper'?'论文 · '+esc(item.venue||'arXiv 预印本'):esc(item.kind||'研究动态')}</span>${!isTopic?`<time datetime="${esc(date)}">${esc(date)}</time>`:''}${saveButton(item)}</div><div class="card-detail"><div class="card-text"><h2><button data-read="${esc(item.id)}">${esc(item.title)}</button></h2>${item.type==='paper'?`<p class="paper-title">${esc(item.paperTitle||'')}</p>`:''}<p class="summary">${esc(item.summary)}</p></div>${image&&!isTopic?`<img class="thumb" src="${esc(image)}" alt="${esc(item.image.alt)}" loading="lazy" referrerpolicy="no-referrer">`:''}</div>${featured?`<div class="highlight"><b>为什么值得关注</b>${esc(item.why)}</div>`:''}${tags(item)}<div class="card-foot"><span class="source">${isTopic?`${related(item).length} 条关联内容`:(item.sources||[]).map(s=>esc(s.name)).join(' · ')}</span><button class="text-button" data-read="${esc(item.id)}">${isTopic?'进入专题':'阅读全文'} ↗</button></div></article>`;}
function related(item){const explicit=new Set(item.relatedIds||[]);return allItems().filter(x=>x.id!==item.id&&(explicit.has(x.id)||((item.type==='topic')&&(x.tags||[]).some(t=>(item.tags||[]).includes(t))))).slice(0,12);}
function rail(){const topics=catalog.topics.slice(0,3);return `<aside class="rail" aria-label="阅读导航"><section class="rail-box"><h2>正在关注的方向</h2><div class="rail-topics">${topics.map((t,i)=>`<button data-read="${esc(t.id)}"><span class="topic-mark">${['◈','◎','↗'][i]}</span><span>${esc(t.title)}<small>${esc(t.subtitle||'研究线索与关联论文')}</small></span></button>`).join('')}</div></section><section class="rail-box"><h2>已积累的阅读</h2><div class="rail-stat"><span>每日简报</span><b>${catalog.days.length} 期</b></div><div class="rail-stat"><span>收录动态</span><b>${catalog.news.length} 条</b></div><div class="rail-stat"><span>论文资料</span><b>${catalog.papers.length} 篇</b></div><p>从一个关键词出发，重新发现读过的内容。</p><div class="tags" style="margin-top:14px">${['快慢脑','World Model','PX4','RAG'].map(t=>`<button class="tag" data-tag="${t}">${t}</button>`).join('')}</div></section><section class="rail-box blue"><h2>值得多看一眼的研究</h2><p>先读核心事实，再看证据边界。把有价值的论文留在自己的收藏里。</p><button class="text-button" data-go="papers">浏览论文库 ↗</button></section></aside>`;}
...
function filterBar(r){if(r.view==='topics')return '';const cat=r.params.get('category')||'';const searchType=r.params.get('type')||'';return `<div class="filters" aria-label="内容筛选">${['',...CATEGORIES].map(c=>`<button class="filter ${cat===c?'active':''}" data-category="${esc(c)}" aria-pressed="${cat===c}">${esc(c||'全部')}</button>`).join('')}<div class="select-group">${r.view==='search'?`<label for="type-filter">内容</label><select id="type-filter"><option value="">全部内容</option>${[['news','新闻'],['paper','论文'],['topic','专题']].map(([v,t])=>`<option value="${v}" ${searchType===v?'selected':''}>${t}</option>`).join('')}</select>`:''}${['latest','search'].includes(r.view)?`<label for="days-filter">时间</label><select id="days-filter">${[['0','全部时间'],['1','最近 1 天'],['3','最近 3 天'],['7','最近 7 天'],['30','最近 30 天']].map(([v,t])=>`<option value="${v}" ${(r.params.get('days')||'0')===v?'selected':''}>${t}</option>`).join('')}</select>`:''}${r.view!=='today'?`<label for="sort-filter">排序</label><select id="sort-filter">${[['newest','最新发布'],['oldest','最早发布'],...(r.view==='search'?[['relevance','相关程度']]:[])].map(([v,t])=>`<option value="${v}" ${(r.params.get('sort')||(r.view==='search'?'relevance':'newest'))===v?'selected':''}>${t}</option>`).join('')}</select>`:''}</div></div>`;}
...
const titles={today:'今日科技简报',latest:'最新动态',history:'历史归档',papers:'论文库',topics:'研究专题',saved:'我的收藏',search:'搜索阅读空间'};
const desc={today:latest===now?'从 AI 到真实世界，读懂今天值得关注的进展。':`今天尚未收录新简报 · 最近一期 ${latest||'暂无'}`,latest:'按原始发布日期浏览，不错过新的研究线索。',history:'每一期独立保存，让读过的内容有迹可循。',papers:'保留原论文、核心方法与证据边界。',topics:'将分散的新闻，连接成清晰的研究方向。',saved:'收藏与笔记保存在当前浏览器，可导出备份。',search:'搜索全部历史新闻、论文、来源与专题。'};
...
if(r.view==='latest')items=catalog.news;
if(r.view==='papers'){items=catalog.papers;layout='grid';}
if(r.view==='topics'){items=catalog.topics;layout='grid';}
...
if(r.view==='history'){
 const month=r.params.get('month')||'';const days=catalog.days.filter(d=>!month||d.date.startsWith(month));
 controls=`<div class="filters"><label for="month-filter">按月份查找</label><input id="month-filter" type="month" value="${esc(/^\d{4}-\d{2}$/.test(month)?month:'')}"><button class="text-button" data-reset>全部日期</button></div>`+controls;
 prefix=`<div class="archive-list">${days.map(d=>`<div class="archive-day"><strong>${esc(d.date.slice(8))}</strong><div><b>${esc(d.date)}</b><p>${d.count} 条简报 · ${esc(d.title||'每日科技简报')}</p></div><button data-day="${esc(d.date)}">${d.date===day?'正在阅读':'查看这期'}</button></div>`).join('')||empty('该月份还没有简报','可以选择其他月份或查看全部日期。')}</div>`;
 items=days.some(d=>d.date===day)?catalog.news.filter(n=>n.date===day):[];
 if(items.length)prefix+=`<div class="section-label">${esc(day)}<span>当天存档</span></div>`;
}
...
if(r.view==='search'){const counts=['news','paper','topic'].map((t,i)=>`${['新闻','论文','专题'][i]} ${filtered.filter(x=>x.type===t).length}`).join(' · ');prefix=`<p class="result-caption">${q?'“'+esc(q)+'” · ':''}${filtered.length} 条结果 · ${counts}</p>`;}
...
$('#results').innerHTML=r.view==='today'?`<div class="content-layout"><div><div class="section-label">本期精选<span>${filtered.length} 条 · ChatGPT 整理</span></div><div class="feed">${list}</div></div>${rail()}</div>`:`${prefix}<div class="${layout}">${list}</div>`;
...
function openReader(id){const item=allItems().find(x=>x.id===id);if(!item)return;activeReader=id;returnFocus=document.activeElement;const image=imageUrl(item),linked=related(item);let body=`<div class="card-top"><span class="badge ${badgeClass(item)}">${esc(item.category)}</span><span>${item.type==='topic'?'专题':esc(item.published||item.date)}</span>${saveButton(item)}</div><h2 id="reader-title">${esc(item.title)}</h2>${item.paperTitle?`<p class="reader-meta">${esc(item.paperTitle)}</p>`:''}${item.authors?.length?`<p class="reader-meta">${esc(item.authors.join('、'))}</p>`:''}<p>${esc(item.summary)}</p>`;
...
body+=`<h3>原始来源</h3><div class="source-links">${(item.sources||[]).filter(s=>safeUrl(s.url)).map(s=>`<a href="${esc(safeUrl(s.url))}" target="_blank" rel="noopener noreferrer">${esc(s.name)} ↗</a>`).join('')}</div><div style="margin-top:20px">${tags(item)}</div>`;
