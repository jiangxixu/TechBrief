...
return `<article class="card ${featured?'featured':''}"><div class="card-top"><span class="badge ${badgeClass(item)}">${esc(item.category)}</span><span>${item.type==='paper'?'论文 · '+esc(item.venue||'arXiv 预印本'):esc(item.kind||'研究动态')}</span>${dateMeta(item)}${saveButton(item)}</div><div class="card-detail"><div class="card-text"><h2><button data-read="${esc(item.id)}">${esc(item.title)}</button></h2>${item.type==='paper'&&item.paperTitle!==item.title?`<p class="paper-title">${esc(item.paperTitle||'')}</p>`:''}<p class="summary">${esc(item.summary)}</p></div>${image?`<img class="thumb" src="${esc(image)}" alt="${esc(item.image.alt)}" loading="lazy" referrerpolicy="no-referrer">`:''}</div>${featured?`<div class="highlight"><b>为什么值得关注</b>${esc(item.why)}</div>`:''}${tags(item)}<div class="card-foot"><span class="source">${(item.sources||[]).map(s=>esc(s.name)).join(' · ')}</span><button class="text-button" data-read="${esc(item.id)}">阅读全文 ↗</button></div></article>`;}
...
const desc={today:latest===now?'本期日期是简报收录日期；卡片中的来源发布日期可能早一天。':`今天尚未收录新简报 · 最近一期 ${latest||'暂无'}`,history:'按日期和分类查看已经发布的每一期新闻。',papers:'每日简报涉及的 arXiv 论文会自动归入这里。',saved:'收藏与笔记保存在当前浏览器，可导出备份。',search:'搜索全部历史新闻、论文和原始来源。'};
