...
if(r.view==='history'){const selected=r.params.get('category')||'',historySort=r.params.get('sort')||'newest';const groups=(selected?[selected]:CATEGORIES).map(category=>{const group=filtered.filter(item=>item.category===category).sort((a,b)=>{const cmp=(a.date||a.published||'').localeCompare(b.date||b.published||'');return historySort==='oldest'?cmp:-cmp;});return group.length?`<section class="history-group"><div class="section-label"><span class="history-title">${esc(category)}</span><span>${group.length} 条</span></div><div class="history-list">${group.map(archiveCard).join('')}</div></section>`:'';}).join('');prefix=`<p class="result-caption">共 ${filtered.length} 条历史新闻 · 默认按最新发布排序</p>`;list=groups||empty('没有找到相关历史新闻','请选择其他分类。');layout='history-groups';}
